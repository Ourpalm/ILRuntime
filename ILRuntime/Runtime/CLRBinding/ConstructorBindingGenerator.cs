    ...
    static class ConstructorBindingGenerator
    {
        internal static string GenerateConstructorRegisterCode(this Type type, ConstructorInfo[] methods, HashSet<MethodBase> excludes)
        {
            ...
            foreach (var i in methods)
            {
                ...
                sb.AppendLine(string.Format("            app.RegisterCLRMethodRedirectionNeo(method, Ctor_{0}_Neo);",idx));
                sb.AppendLine("#else");
        ...
        static void GenerateConstructorWraperCode_Neo(StringBuilder sb, Type type, ConstructorInfo i, int idx, string typeClsName, List<Type> valueTypeBinders, bool isMultiArr, ParameterInfo[] param)
        {
            sb.AppendLine(string.Format("        static void Ctor_{0}_Neo(ILIntepreter __intp, byte* __frameBase, AutoList __mStack, CLRMethod __method, bool isNewObj, byte* __retDst, int __retRefBase)", idx));
            sb.AppendLine("        {");
            ...
            if (isMultiArr)
            {
                ...
                sb.Append(string.Format("            {0} result_of_this_method = new {1}[", typeClsName, realClsName));
                for (int j = 0; j < param.Length; j++) 
                { 
                    if (j > 0) sb.Append(", ");
                    sb.Append("a" + (j + 1)); 
            ...
            {
                sb.Append(string.Format("            {0} result_of_this_method = new {0}(", typeClsName));
                for (int j = 0; j < param.Length; j++) 
                { 
                    if (j > 0) sb.Append(", ");
        ...
        static void GenerateConstructorWraperCode_Legacy(StringBuilder sb, Type type, ConstructorInfo i, int idx, string typeClsName, List<Type> valueTypeBinders, bool isMultiArr, ParameterInfo[] param, int paramCnt)
        {
        ...
        internal static string GenerateConstructorWraperCode(this Type type, ConstructorInfo[] methods, string typeClsName, HashSet<MethodBase> excludes, List<Type> valueTypeBinders)
        {
            ...
            foreach (var i in methods)
            {
                ...
                sb.AppendLine("#if ENABLE_NEO_MODE");
                GenerateConstructorWraperCode_Neo(sb, type, i, idx, typeClsName, valueTypeBinders, isMultiArr, param);
                sb.AppendLine("#else");
                GenerateConstructorWraperCode_Legacy(sb, type, i, idx, typeClsName, valueTypeBinders, isMultiArr, param, paramCnt);
                sb.AppendLine("#endif");

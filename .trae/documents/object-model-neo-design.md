...
### 3.1 核心设计：Caller-Write + Callee-Own-Refs

**基本原则**：调用者直接将参数写入 `esp`（即被调方的帧起始位置），被调方从帧头部直接读取参数开始执行。引用类型参数以 mStack index 形式传递（与 `ILTypeInstance` 中引用字段在 byte[] 中的存储方式一致），callee 无需额外操作即可直接使用。
...
Caller:
  ...
  2. 调用 ExecuteNeo(targetMethod, esp)
  3. 从 esp + 0 读取返回值（primitive 写值 / reference 写 index）
...
Callee:
  ...
  5. 执行方法体 ...
  6. Ret:
     - 将返回值写到 frameBase + 0（覆盖 param0 位置）
...
帧 byte* 区域中，引用类型槽位存储的是 **mStack 中的 index**。这与 `ILTypeInstance.Primitives` 中引用字段的存储方式一致——byte 区域存 index，实际对象在 mStack（或 ManagedObjects）中。
...
返回值写在 `frameBase + 0`（帧头部），覆盖 param0 位置：
- 方法返回后 params 不再需要
- Caller 从自己的 esp + 0 直接读取，无需额外空间
- void 方法不写任何东西
...
| 返回值 | CopyToStack → PopToRegister | 一次内存读 |
...
| param0 | param1 | ... | slot0 | slot1 | slot2 | ... | slotN |
  4B       8B              所有 locals 和 temps 按实际大小紧凑排列
```

- `esp` 语义变为 `byte*`（纯数据指针），不再是 `StackObject*`
- 帧大小 = `CompiledFrame.TotalStructSize`（编译期已知的所有原始数据总字节数）
...
| 方面 | 旧签名 | Neo 签名 |
|------|--------|----------|
| 返回类型 | `StackObject*`（新 esp 位置）| `void`（返回值已写 frameBase+0）|
...
| 写返回值 | push 到 esp 然后返回 esp+1 | 写到 `frameBase + 0`（primitive）或 `*(int*)(frameBase+0) = mStack.Count; mStack.Add(obj)`（reference）|
| frameRefBase | 不需要 | 不需要（mStack.Count 直接可得）|
...
unsafe static void MathfMax_Neo(ILIntepreter intp, byte* frameBase, AutoList mStack, CLRMethod method, bool isNewObj)
{
    ...
    *(int*)(frameBase + 0) = Math.Max(a, b);  // 返回值覆盖 param0
}
```
...
| Ret 返回值类型 | Ret 指令内部 CopyBlock 到 frameBase+0 |
